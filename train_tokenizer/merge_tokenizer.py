...
parser.add_argument(
    "--llama_tokenizer_dir", default="./llama", type=str, required=False
)
parser.add_argument(
    "--chinese_sp_model_file", default=f"./{model_file}", type=str, required=False
)
...
output_hf_dir = (
    f"merged_tokenizer_hf_{vocab_size}"  # the path to save Chinese-LLaMA tokenizer
)
...
print
print(
    f"Tokenized by LLaMA tokenizer:{len(llama_tokenizer.tokenize(text))},{llama_tokenizer.tokenize(text)}"
)
print(
