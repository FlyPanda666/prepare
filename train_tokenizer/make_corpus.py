...
# https://zhuanlan.zhihu.com/p/532294408
...
def process_cls():
    ...
    cnt = 0
    with open(
        "/data/searchgpt/data/corpus/CSL/csl_camera_readly.tsv", "r", encoding="utf-8"
    ) as f:
...
def process_lcsts():
    ...
    cnt = 0
    with open(
        "/data/searchgpt/data/corpus/LCSTS_new/train.json", "r", encoding="utf-8"
    ) as f:
        for line in f.readlines():
    ...
    with open(
        "/data/searchgpt/data/corpus/LCSTS_new/dev.json", "r", encoding="utf-8"
    ) as f:
        for line in f.readlines():
            data = json.loads(line.strip())
            corpus_lcsts.write(data["summary"] + " " + data["content"] + "\n")
